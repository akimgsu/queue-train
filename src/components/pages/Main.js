...
export default class Main extends Component {
    ...
    connectToWebScoket = () => {
        const address = `wss://${process.env.REACT_APP_SOCKET_API_GATEWAY_ID}.execute-api.ap-northeast-2.amazonaws.com/dev`
    ...
    onMessageReceived = async (message) => {
        ...
        if (message.status == "start") {
            ...
            this.timer = setInterval(() => {
                that.setState({
                    view_time: that.state.view_time - 1000
                })
            }, 1000);
        }
    ...
    onSubmit = async (message) => {
        ...
        const result = await axios({
            method: 'DELETE',
            url: `https://${process.env.REACT_APP_API_GATEWAY_ID}.execute-api.ap-northeast-2.amazonaws.com/dev/sqs`,
            params: {
